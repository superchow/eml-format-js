define(['exports', 'js-base64', 'text-encoding'], function (exports, jsBase64, textEncoding) { 'use strict';
	...
	function parse(eml, options, callback) {
	    ...
	    try {
	        ...
	        result = parseRecursive(lines, 0, options);
	    }
	    catch (e) {
	        error = e;
	    }
	 ...
	 */
	function parseRecursive(lines, start, options) {
	    var result = {
	        headers: {},
	    };
	    var boundary = {
	            ...
	            if (isMultipart && Array.isArray(result.body)) {
	                ...
	                if (insideBoundary && boundary) {
	                    ...
	                    if (boundary.boundary && lines[i + 1] === '' && line.indexOf('--' + findBoundary + '--') === 0) {
	                        ...
	                        result.body = result.body.map(function (boundaryRawData) { return completeBoundary(boundaryRawData); }).filter(function (a) { return a; });
	                        continue;
	                    }
